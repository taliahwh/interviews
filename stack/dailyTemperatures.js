...
const dailyTemperatures = (temperatures) => {
  ...
  const stack = []; // [futureDay, index]

  // iterate through array backwards
  for (let i = temperatures.length - 1; i >= 0; i--) {
    while (stack.length) {
      const [futureDay, index] = stack[stack.length - 1];

      if (futureDay <= temperatures[i]) {
        stack.pop();
      } else {
        // found a warmer day
        answer[i] = index - i;
    ...
    // if stack is empty, there is no future date with a warmer temp
    if (!stack.length) {
      answer[i] = 0;
      stack.push([temperatures[i], i]);
    }
